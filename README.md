...
Deployed at: https://Invertisment.github.io/create-sablier-stream
...
* [`src/cljs/multis_task/`](src/cljs/multis_task/): SPA source files (ClojureScript,
[re-frame](https://github.com/Day8/re-frame))
  - [`core.cljs`](src/cljs/multis_task/core.cljs): contains the SPA entry point, `init`
...
7. Clone this repo and open a terminal in the `multis-task` project root directory
...
The `debug?` variable in [`config.cljs`](src/cljs/multis_task/config.cljs) defaults to `true` in
[`dev`](#running-the-app) builds, and `false` in [`prod`](#production) builds.

Use `debug?` for logging or other tasks that should run only on `dev` builds:

```clj
(ns multis-task.example
  (:require [multis-task.config :as config])
